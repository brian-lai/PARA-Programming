...
4. Creates a new git branch named `pret/{task-name}` or `pret/{task-name}-phase-N`
...
```
/pret-execute --phase=1               # Execute specific phase
/pret-execute --phase=2               # Execute next phase
/pret-execute                         # Will prompt for which phase to execute
```

For phased plans:
- Each phase creates a separate branch: `pret/{task-name}-phase-N`
...
```
/pret-execute --branch=custom-name    # Use custom branch name (simple plans only)
/pret-execute --no-branch             # Skip branch creation (continue on current branch)
/pret-execute --phase=N               # Execute specific phase (phased plans only)
```
...
1. **Plan** - `/pret-plan` creates the plan (done before this)
2. **Review** - Human validates the approach (done before this)
3. **Execute** - `/pret-execute` sets up execution tracking ← YOU ARE HERE
4. **Summarize** - `/pret-summarize` captures results (after work is done)
5. **Archive** - `/pret-archive` cleans up (after summarizing)

### Phased Plan Workflow

For each phase:

1. **Plan** - `/pret-plan` creates master + sub-plans (done before this)
2. **Review** - Human validates all phases (done before this)
3. **Execute Phase N** - `/pret-execute --phase=N` sets up phase tracking ← YOU ARE HERE
4. **Implement** - Work through phase to-dos, commit incrementally
5. **Summarize Phase N** - `/pret-summarize --phase=N` captures phase results
6. **PR & Merge** - Create PR, get review, merge to main
7. **Repeat** - Move to next phase
8. **Archive** - `/pret-archive` cleans up after all phases complete
...
4. For simple plans:
   ...
   - If no active plan, error with: "No active plan found. Run `/pret-plan` first."
5. For phased plans:
...
3. Check if target branch already exists:
   - If exists, ask: "Branch `pret/{task-name}` exists. Continue on it or create new?"
...
1. Generate branch name: `pret/{task-name}`
2. Run: `git checkout -b pret/{task-name}`
3. If branch creation fails, report error

#### For Phased Plans

1. Ensure we're on `main` branch first: `git checkout main && git pull`
2. Generate branch name: `pret/{task-name}-phase-{N}`
3. Run: `git checkout -b pret/{task-name}-phase-{N}`
...
```json
{
  ...
  "completed_summaries": [],
  "execution_branch": "pret/{task-name}-phase-{N}",
  "execution_started": "{ISO timestamp}",
  "phased_execution": {
...
3. When all items are complete, run `/pret-summarize`
...
```
⚠️ Branch `pret/add-auth` already exists.

Options:
1. Continue on existing branch (recommended if resuming work)
2. Create new branch with suffix: `pret/add-auth-2`
3. Cancel
...
- Branch naming follows `pret/{task-name}` convention for easy identification
...
- Run `/pret-status` anytime to see current progress
